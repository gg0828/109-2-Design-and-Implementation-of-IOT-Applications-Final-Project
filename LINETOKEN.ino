...
int t,h;
//int frequency = 1000;
//int ton = 500,toff=500;
int count = 0;
//String temp,humid,soil;
//long i=0;
void loop() {
    if (digitalRead(0) == LOW) clr_eeprom();
    ...
    if (millis() - cycleTimestamp > 200) {
        ...
        int t = DhtSensor.temperature , h = DhtSensor.humidity;
        Serial.println(String(t)+"/"+String(h));
        Serial.println("Soil_moisture: "+String(pinA0));
        if(pinA0>800){
            if(count==0){
              digitalWrite(D8,HIGH);//繼電器
              digitalWrite(D1,HIGH);delay(200);//RED
              digitalWrite(D2,HIGH);delay(200);//GREEN
              digitalWrite(D6,HIGH);delay(300);//BLUE
              digitalWrite(D8,LOW);
            }count=count+1;
        }else{
            digitalWrite(D1,LOW);
        ...
        //pinA0 = map(pinA0,0,1023,100,0);
        /*tone(D0, frequency);
        delay(ton);
        noTone(D0);
        delay(toff);*/
        push("A0", String(t)+String(h)+String(pinA0));
        delay(2000);
